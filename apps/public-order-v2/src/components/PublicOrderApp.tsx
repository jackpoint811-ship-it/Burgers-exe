import { type MenuItem, type MenuV2Response, siteConfig as mockSiteConfig } from '@config/index';
...
const CatalogImage = ({ src, alt, fallbackLabel, kind }: CatalogImageProps) => {
  ...
  return <div className='catalog-visual'>{showImage ? <img className='catalog-image' src={src} alt={alt} loading='lazy' decoding='async' onError={() => setFailedSrc(src ?? null)} /> : <PlaceholderVisual label={fallbackLabel} kind={kind} />}</div>;
};
...
const HeroSection = ({ sourceLabel }: { sourceLabel: string }) => <Card className='hero-card rounded-3xl p-5 sm:p-8'><Badge className='w-fit border-white/20 bg-white/5 text-zinc-200'>{sourceLabel}</Badge><div className='mt-4 grid items-center gap-5 md:grid-cols-[1.2fr_1fr]'><article><p className='text-xs uppercase tracking-[0.28em] text-neon'>Burgers.exe live batch</p><h1 className='mt-2 text-3xl font-black leading-tight sm:text-5xl'>Smash burgers que aterrizan en minutos.</h1><p className='mt-3 max-w-xl text-sm text-zinc-200 sm:text-base'>Sube el antojo en 3 pasos: elige un combo viral, agrega extras y simula tu ticket premium al instante.</p><div className='mt-5 flex flex-wrap gap-3'><a href='#menu' className='rounded-xl bg-neon px-5 py-3 text-sm font-bold text-black sm:text-base'>Quiero mi burger</a><a href='#promos' className='rounded-xl border border-white/30 bg-black/20 px-5 py-3 text-sm font-semibold sm:text-base'>Explorar promos</a></div></article><PlaceholderVisual label='Signature combo' kind='combo' /></div></Card>;

const PromoCard = ({ promo, reduce, onPromoAction, canAddDirectly }: PromoCardProps) => <motion.article whileHover={reduce ? undefined : { y: -4 }} className='rounded-3xl border border-white/15 bg-zinc-900/80 p-5'><div className='mb-3 flex items-center justify-between text-xs uppercase tracking-widest text-zinc-300'><Badge className='border-neon/40 text-neon'>{promo.badge ?? 'Top pick'}</Badge><span>{promo.promoLabel ?? 'Limited'}</span></div><h3 className='text-2xl font-extrabold leading-tight sm:text-3xl'>{promo.title}</h3><p className='mt-2 text-sm text-zinc-300 sm:text-base'>{promo.description}</p><div className='mt-4'><CatalogImage src={resolveAssetUrl(promo.asset.imageUrl, promo.asset.imageKey)} alt={promo.asset.alt} fallbackLabel={promo.asset.alt} kind={getVisualKind(promo.asset.placeholder)} /></div><Button onClick={() => onPromoAction(promo)} className='mt-4 w-full bg-white text-black'>{canAddDirectly ? 'Agregar promo al ticket' : 'Ver en menú'}</Button></motion.article>;
const PromoSection = ({ promos, reduce, onPromoAction, canAddPromo }: { promos: PromoEntry[]; reduce: boolean; onPromoAction: (promo: PromoEntry) => void; canAddPromo: (promo: PromoEntry) => boolean }) => <section id='promos' className='space-y-3'><SectionHeader title='Promos grandes, hambre resuelta' subtitle='Descuentos, combos y favoritos del turno.' /><div className='grid gap-4 md:grid-cols-2'>{promos.map((promo) => <PromoCard key={promo.id} promo={promo} reduce={reduce} onPromoAction={onPromoAction} canAddDirectly={canAddPromo(promo)} />)}</div></section>;

const MenuItemCard = ({ item, onAdd, reduce }: MenuItemCardProps) => <motion.article whileTap={reduce ? undefined : { scale: 0.98 }} className='rounded-2xl border border-white/15 bg-zinc-900/80 p-3'><CatalogImage src={resolveAssetUrl(item.imageUrl, item.imageKey)} alt={item.name} fallbackLabel={item.name} kind={getVisualKind(item.tags.join('-'))} /><div className='mt-3 flex items-start justify-between gap-2'><div><h4 className='font-semibold'>{item.name}</h4><p className='text-xs uppercase tracking-widest text-zinc-400'>{item.badge ?? item.promoLabel ?? 'Hecho al momento'}</p></div><span className='text-lg font-black text-neon'>{formatCurrency(item.price)}</span></div><p className='mt-1 text-sm text-zinc-300'>{item.description}</p><Button disabled={!item.isAvailable} onClick={() => onAdd(item)} className='mt-3 w-full bg-neon text-black disabled:bg-zinc-800 disabled:text-zinc-500'>{item.isAvailable ? 'Agregar al ticket' : 'Agotado por ahora'}</Button></motion.article>;
...
const CheckoutMock = ({ customer, setCustomer, onCheckout }: { customer: CustomerDraft; setCustomer: (v: CustomerDraft) => void; onCheckout: () => void }) => <div className='space-y-3 pt-2'><label className='text-xs uppercase tracking-widest text-zinc-300'>Nombre<input className='mt-1.5 w-full rounded-lg border border-white/20 bg-zinc-900 px-3 py-2.5 text-sm' placeholder='Tu nombre' value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} /></label><label className='text-xs uppercase tracking-widest text-zinc-300'>Teléfono<input className='mt-1.5 w-full rounded-lg border border-white/20 bg-zinc-900 px-3 py-2.5 text-sm' placeholder='55 0000 0000' value={customer.phone} onChange={(e) => setCustomer({ ...customer, phone: e.target.value })} /></label><label className='text-xs uppercase tracking-widest text-zinc-300'>Entrega<select className='mt-1.5 w-full rounded-lg border border-white/20 bg-zinc-900 px-3 py-2.5 text-sm' value={customer.payment} onChange={(e) => setCustomer({ ...customer, payment: e.target.value as Payment })}><option value='pickup'>Pickup</option><option value='delivery'>Delivery</option></select></label><label className='text-xs uppercase tracking-widest text-zinc-300'>Notas<textarea className='mt-1.5 w-full rounded-lg border border-white/20 bg-zinc-900 px-3 py-2.5 text-sm' rows={2} placeholder='Alergias, sin cebolla, etc.' value={customer.notes} onChange={(e) => setCustomer({ ...customer, notes: e.target.value })} /></label><Button onClick={onCheckout} className='w-full bg-white py-3 font-bold text-black'>Simular pedido</Button></div>;

const CartPanel = ({ cart, total, count, onMinus, onPlus, customer, setCustomer, onCheckout, success, items }: CartPanelProps & { items: MenuItem[] }) => <aside className='sticky top-2 h-fit space-y-3 rounded-2xl border border-white/15 bg-zinc-950 p-4'><p className='text-xs uppercase tracking-[0.2em] text-zinc-400'>Acciones locales · no se envía a producción</p><h3 className='text-xl font-bold'>Ticket ({count})</h3><div className='max-h-56 space-y-2 overflow-auto'>{cart.length === 0 ? <EmptyState title='Tu ticket está vacío' description='Agrega un combo o burger para comenzar.' /> : cart.map((entry) => {const item = items.find((m) => m.sku === entry.sku);if (!item) return null;return <div key={entry.sku} className='rounded-lg border border-dashed border-white/20 bg-zinc-900/80 p-2 text-sm'><div className='flex justify-between'><span>{item.name}</span><span>{formatCurrency(item.price * entry.qty)}</span></div><QuantityControl qty={entry.qty} onMinus={() => onMinus(entry.sku)} onPlus={() => onPlus(entry.sku)} /></div>;})}</div><p className='border-t border-white/10 pt-3 text-3xl font-black'>Total {formatCurrency(total)}</p><CheckoutMock customer={customer} setCustomer={setCustomer} onCheckout={onCheckout} />{success ? <p className='rounded-lg border border-emerald-400/40 bg-emerald-500/15 p-2 text-sm text-emerald-200'>Orden simulada: cocina en preparación local.</p> : null}</aside>;

const MenuSection = ({ categories, items, onAdd, reduce, ...cartProps }: MenuSectionProps & { categories: MenuV2Response['categories']; items: MenuItem[] }) => <section id='menu' className='grid gap-5 lg:grid-cols-[1fr_340px]'><div className='space-y-4'><SectionHeader title='Menú smash-ready' subtitle='Sabor primero: badges claros, precios visibles y CTA directo.' />{categories.map((category) => <MenuCategoryBlock key={category.id} name={category.name} items={items.filter((item) => item.category === category.key)} onAdd={onAdd} reduce={reduce} />)}</div><CartPanel {...cartProps} items={items} /></section>;
...
export function PublicOrderApp() {
  ...
  const sourceLabel = menuData.source === 'd1' ? 'Catálogo live' : menuData.source === 'mock' ? 'Catálogo local' : 'Fallback local';

  return <main className='mx-auto max-w-7xl space-y-8 px-3 py-4 sm:px-6 lg:px-8'><HeroSection sourceLabel={sourceLabel} />{loadingMenu ? <p className='text-sm text-zinc-400'>Cargando catálogo…</p> : null}<PromoSection promos={menuData.promos} reduce={reduce} onPromoAction={handlePromoAction} canAddPromo={canAddPromo} /><MenuSection categories={menuData.categories} items={menuData.items} cart={cart} total={total} count={count} onAdd={addToCart} reduce={reduce} onMinus={(sku: string) => updateQty(sku, -1)} onPlus={(sku: string) => updateQty(sku, 1)} customer={customer} setCustomer={setCustomer} onCheckout={() => setSuccess(true)} success={success} /><TrustSection /><p className='text-center text-xs text-zinc-500'>{menuData.siteConfig.notice || mockSiteConfig.notice}</p></main>;
}
