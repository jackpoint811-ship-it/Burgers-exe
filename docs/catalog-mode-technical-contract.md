...
`MenuItem.category` ya distingue `burgers`, `combos`, `extras`, `guarniciones` y `drinks`. Sin embargo, el Modo Catálogo necesita un `ProductType` explícito (`burger | combo | side | topping | drink`) y reglas de edición/visibilidad separadas. Hoy los toppings se parecen más a `extras`; guarniciones usan `guarniciones`; combos existen como categoría y/o promos. Se recomienda mapear sin romper:
...
```ts
type ProductType = "burger" | "combo" | "side" | "topping" | "drink";
...
- `packages/config` debe exponer `PublicMode`, `PublicConfig`, `CatalogSettings`, `DEFAULT_PUBLIC_CONFIG`, `DEFAULT_CATALOG_SETTINGS`, `resolvePublicConfig` y `shouldUseCatalogMode`.
- El default obligatorio es `{ publicMode: "flow", catalogEnabled: false }`; esto impide activar Modo Catálogo por accidente.
- La ruta más segura para PR 2 es incluir `publicConfig` en el payload de `/api/menu-v2` como campo aditivo con fallback local, sin migrar `site_config` todavía.
- Public Order V2 puede preparar una bifurcación mínima con `shouldUseCatalogMode(publicConfig)`, pero debe renderizar el Modo Flujo actual mientras el default siga en `flow` y `catalogEnabled=false`.
- Chekeo/Admin no debe exponer todavía un toggle operativo que active catálogo en producción; la edición protegida queda para PR posterior con auditoría y persistencia definida.
