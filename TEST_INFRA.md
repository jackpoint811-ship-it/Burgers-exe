...
We focus on three primary target features under test:
...
To execute the specific E2E test suite under the Public Order configuration:
```bash
npx playwright test tests/e2e-catalog-kitchen.spec.ts --config=playwright.visual.config.ts
```

To execute the specific E2E test suite under the Kitchen Board configuration:
```bash
