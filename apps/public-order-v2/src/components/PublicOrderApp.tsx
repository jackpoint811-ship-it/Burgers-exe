...
const LoadingOverlay = ({ loading }: { loading: boolean }) => loading ? (
  <div className="boot-overlay" role="status" aria-live="polite">
    <div className="boot-window">
      <h1>INITIALIZING BURGERS.EXE...</h1>
      <p>&gt; LOADING MENU_ASSETS... <strong>[OK]</strong></p>
      <p>&gt; MOUNTING WORKBENCH... <strong>[OK]</strong></p>
      <p>&gt; SYNCING QUEST_FLOW... <strong>[OK]</strong></p>
      <p>&gt; SYSTEM READY.</p>
  ...
  return (
    ...
    >
      <div className="kiosk-visual">
        {src ? <img src={src} alt="" loading="lazy" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : <span>{item.name}</span>}
      </div>
      ...
      {MENU_GROUPS.map(({ key, label }) => {
        ...
        return (
          <section className="menu-cluster" key={key}>
            <h2>{label}</h2>
            {list.length ? <div className="kiosk-grid">{list.map((item) => <ProductCard key={item.sku} item={item} mode="info" onClick={onExplore} reduce={reduce} />)}</div> : <EmptyState title={`${label}: sin productos configurados`} description="Esta sección solo muestra datos reales del catálogo." />}
          </section>
        );
      })}
    </section>
...
const UnitEditor = ({ unit, index, item, extras, garnishes, onChange }: { unit: CartEntry; index: number; item: MenuItem; extras: MenuItem[]; garnishes: MenuItem[]; onChange: (unit: CartEntry) => void }) => {
  ...
  return (
    <article className="unit-editor">
      ...
      {isBurgerLike ? <div className="builder-block"><h4>MOD</h4>{ingredients.length ? <div className="chip-grid">{ingredients.map((ingredient) => {
        const active = unit.removedIngredients.includes(ingredient);
        return <button type="button" key={ingredient} className={active ? "chip active" : "chip"} onClick={() => onChange({ ...unit, removedIngredients: active ? unit.removedIngredients.filter((entry) => entry !== ingredient) : [...unit.removedIngredients, ingredient] })}>MOD · Sin {ingredient}</button>;
      })}</div> : <p className="muted">Sin ingredientes editables configurados para esta burger.</p>}</div> : null}
      {isBurgerLike ? <div className="builder-block"><h4>UPGRADE</h4>{extras.length ? <div className="chip-grid">{extras.map((extra) => {
        const active = unit.extras.some((entry) => entry.sku === extra.sku);
        return <button type="button" key={extra.sku} className={active ? "chip active" : "chip"} onClick={() => onChange({ ...unit, extras: active ? unit.extras.filter((entry) => entry.sku !== extra.sku) : [...unit.extras, { sku: extra.sku, name: extra.name, price: extra.price }] })}>UPGRADE · {extra.name} +{formatCurrency(extra.price)}</button>;
      })}</div> : <p className="muted">Sin extras configurados.</p>}</div> : null}
      {unit.itemKind === "combo" ? <div className="builder-block"><h4>Guarnición incluida</h4>{garnishes.length ? <div className="chip-grid">{garnishes.map((garnish) => <button type="button" key={garnish.sku} className={unit.garnish?.sku === garnish.sku ? "chip active" : "chip"} onClick={() => onChange({ ...unit, garnish: { sku: garnish.sku, name: garnish.name } })}>{garnish.name}</button>)}</div> : <p className="inline-error">No hay guarniciones configuradas en el catálogo real.</p>}</div> : null}
...
const SideQuest = ({ garnishes, selected, onToggle, onBack, onSkip, reduce }: { garnishes: MenuItem[]; selected: string[]; onToggle: (sku: string) => void; onBack: () => void; onSkip: () => void; reduce: boolean }) => (
  <section className="quest-panel">
    ...
    <p className="muted">La guarnición incluida de combo ya viaja dentro del combo. Aquí todo se agrega como línea separada con precio propio.</p>
    {garnishes.length ? <div className="kiosk-grid">{garnishes.map((item) => <div className={selected.includes(item.sku) ? "side-card active" : "side-card"} key={item.sku}><ProductCard item={item} mode="select" onClick={() => onToggle(item.sku)} reduce={reduce} /></div>)}</div> : <EmptyState title="Sin guarniciones configuradas" description="Puedes saltar Side Quest sin inventar productos." />}
    <QuestButton className="ghost" onClick={onSkip}>Saltar Side Quest</QuestButton>
  </section>
);
...
const Checkout = ({ cart, items, total, customer, setCustomer, onBack, onSubmit, submitting, error, onEdit, onDuplicate, onRemove }: { cart: CartEntry[]; items: MenuItem[]; total: number; customer: CustomerDraft; setCustomer: (customer: CustomerDraft) => void; onBack: () => void; onSubmit: () => void; submitting: boolean; error: string | null; onEdit: (lineKey: string) => void; onDuplicate: (lineKey: string) => void; onRemove: (lineKey: string) => void }) => (
  <section className="quest-panel checkout-panel">
    ...
    <QuestButton onClick={onSubmit} disabled={submitting || !cart.length}>{submitting ? "ENVIANDO PEDIDO..." : "EJECUTAR PEDIDO"}</QuestButton>
...
export function PublicOrderApp() {
  ...
  const confirmBuilder = () => {
    ...
    if (builder.itemKind === "combo" && builder.units.some((unit) => !unit.garnish)) {
      setBuilder({ ...builder, error: garnishes.length ? "El combo requiere elegir guarnición incluida." : "No hay guarniciones configuradas para confirmar este combo." });
      return false;
    }
