...
export function CatalogCartDrawer({ isOpen, onClose, onCheckout, sides = [], isStaticPanel = false }: CatalogCartDrawerProps) {
  ...
  useEffect(() => {
    if (!isOpen || isStaticPanel) return;
  ...
  if (isStaticPanel) {
    return (
      <section className="catalog-cart-drawer static-panel" aria-labelledby={titleId}>
        {innerContent}
      </section>
    );
  }
