...
function buildFallback(warnings) {
  return {
    ok: true,
    source: 'fallback',
...
export async function onRequest(context) {
  ...
  const endpoint = context.env && context.env.APPS_SCRIPT_MENU_ENDPOINT;
  if (!endpoint) {
    return jsonResponse(200, buildFallback(['APPS_SCRIPT_MENU_ENDPOINT no configurado; usando menú estático fallback.']));
  }
  ...
  try {
    ...
    return jsonResponse(200, buildFallback(warnings));
  } catch (error) {
    return jsonResponse(200, buildFallback(['No se pudo consultar Apps Script; usando menú estático fallback.', String(error && error.message ? error.message : 'Error desconocido')]));
  }
}
