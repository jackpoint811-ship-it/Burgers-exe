...
type CatalogBannerForm = { id?: string; title: string; subtitle: string; ctaLabel: string; isActive: boolean; sortOrder: string; imageUrl: string; imageKey: string };
type CatalogBannerMutationResponse = { ok?: boolean; error?: string; warning?: string; banner?: CatalogBanner; imageKey?: string | null; assetUrl?: string | null; removed?: boolean };
...
export function CatalogAdminPanel() {
  ...
  const [catalogBannerForm, setCatalogBannerForm] = useState<CatalogBannerForm | null>(null);
  const [catalogBannerSaving, setCatalogBannerSaving] = useState(false);
  const [catalogBannerUploading, setCatalogBannerUploading] = useState(false);
  const [catalogBannerRemovingImage, setCatalogBannerRemovingImage] = useState(false);
  ...
  const beginCreateCatalogBanner = () => {
    setCatalogBannerForm({ title: '', subtitle: '', ctaLabel: '', isActive: true, sortOrder: '0', imageUrl: '', imageKey: '' });
    setSelectedCatalogBannerFile(null);
    setCatalogBannerError(null);
    if (catalogBannerFileInputRef.current) catalogBannerFileInputRef.current.value = '';
  };

  const beginEditCatalogBanner = (banner: CatalogBanner) => {
    setCatalogBannerForm({ id: banner.id, title: banner.title, subtitle: banner.subtitle || '', ctaLabel: banner.ctaLabel || '', isActive: banner.isActive, sortOrder: String(banner.sortOrder), imageUrl: banner.imageUrl || '', imageKey: banner.imageKey || '' });
    setSelectedCatalogBannerFile(null);
    setCatalogBannerError(null);
    if (catalogBannerFileInputRef.current) catalogBannerFileInputRef.current.value = '';
  };

  const closeCatalogBannerEditor = () => {
    if (catalogBannerSaving || catalogBannerUploading || catalogBannerRemovingImage) return;
    setCatalogBannerForm(null);
    setSelectedCatalogBannerFile(null);
    setCatalogBannerError(null);
  };

  const updateCatalogBannerInMenu = (banner?: CatalogBanner, removedId?: string) => {
    setMenu((current) => {
      if (!current) return current;
      let newBanners = [...(current.catalogBanners ?? [])];
      if (removedId) {
        newBanners = newBanners.filter(b => b.id !== removedId);
      } else if (banner) {
        const idx = newBanners.findIndex(b => b.id === banner.id);
        if (idx >= 0) newBanners[idx] = banner;
  ...
  const onCatalogBannerSave = async () => {
    if (!canEdit || catalogBannerSaving || !catalogBannerForm) return;
    const { id, title, subtitle, ctaLabel, isActive, sortOrder } = catalogBannerForm;
    if (!title.trim()) { setCatalogBannerError('El título es requerido'); return; }
    if (!Number.isInteger(Number(sortOrder))) { setCatalogBannerError('Orden debe ser un número entero'); return; }
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        credentials: 'include',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ title, subtitle: subtitle || null, ctaLabel: ctaLabel || null, isActive, sortOrder: Number(sortOrder) })
      });
  ...
  const onCatalogBannerDelete = async () => {
    if (!canEdit || catalogBannerSaving || !catalogBannerForm?.id) return;
    if (!confirm('¿Seguro que deseas eliminar este banner permanentemente?')) return;
    setCatalogBannerSaving(true);
    setCatalogBannerError(null);
    try {
      const res = await fetch(`/api/menu-v2-admin/catalog-banners/${encodeURIComponent(catalogBannerForm.id)}`, { method: 'DELETE', credentials: 'include' });
      const data = await res.json() as { ok: boolean; error?: string };
      if (!res.ok || !data.ok) throw new Error(data.error ?? 'Error al eliminar banner');
      updateCatalogBannerInMenu(undefined, catalogBannerForm.id);
      setNotice('Banner eliminado');
      closeCatalogBannerEditor();
    } catch (e) {
      setCatalogBannerError(e instanceof Error ? e.message : 'Error al eliminar banner');
    } finally {
      setCatalogBannerSaving(false);
    }
  };

  const onCatalogBannerUploadImage = async () => {
    if (!canEdit || catalogBannerUploading || !catalogBannerForm?.id) return;
    const fileError = validateSelectedFile(selectedCatalogBannerFile);
    if (fileError || !selectedCatalogBannerFile) { setCatalogBannerError(fileError); return; }
    
    setCatalogBannerUploading(true);
    setCatalogBannerError(null);
    try {
      const body = new FormData();
      body.append('file', selectedCatalogBannerFile);
      const res = await fetch(`/api/menu-v2-admin/catalog-banners/${encodeURIComponent(catalogBannerForm.id)}/image`, { method: 'POST', credentials: 'include', body });
      const data = (await res.json()) as CatalogBannerMutationResponse;
      if (!res.ok || !data.ok || !data.banner) throw new Error(data.error ?? 'No se pudo subir la imagen del banner');
      updateCatalogBannerInMenu(data.banner);
      setSelectedCatalogBannerFile(null);
      if (catalogBannerFileInputRef.current) catalogBannerFileInputRef.current.value = '';
      setNotice(data.warning ? `Imagen actualizada (${data.warning})` : 'Imagen actualizada');
    } catch (e) {
      setCatalogBannerError(e instanceof Error ? e.message : 'No se pudo subir la imagen del banner');
    } finally {
      setCatalogBannerUploading(false);
    }
  };

  const onCatalogBannerRemoveImage = async () => {
    if (!canEdit || catalogBannerRemovingImage || !catalogBannerForm?.id) return;
    setCatalogBannerRemovingImage(true);
    setCatalogBannerError(null);
    try {
      const res = await fetch(`/api/menu-v2-admin/catalog-banners/${encodeURIComponent(catalogBannerForm.id)}/image`, { method: 'DELETE', credentials: 'include' });
      const data = (await res.json()) as CatalogBannerMutationResponse;
      if (!res.ok || !data.ok || !data.banner) throw new Error(data.error ?? 'No se pudo quitar la imagen');
      updateCatalogBannerInMenu(data.banner);
      setSelectedCatalogBannerFile(null);
      if (catalogBannerFileInputRef.current) catalogBannerFileInputRef.current.value = '';
      setNotice(data.warning ? `Imagen quitada (${data.warning})` : 'Imagen quitada');
    } catch (e) {
      setCatalogBannerError(e instanceof Error ? e.message : 'No se pudo quitar la imagen');
    } finally {
      setCatalogBannerRemovingImage(false);
    }
  };
